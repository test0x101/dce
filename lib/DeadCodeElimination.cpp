...
bool RADeadCodeElimination::eliminate_unconditional_branch(Function &F) {
  ...
  while (!Q.empty()) {
    ...
    bb_before->getTerminator()->setSuccessor(
        j, bb->getTerminator()->getSuccessor(0));

    if (!bb->getTerminator()->use_empty()) {
      bb->getTerminator()->replaceAllUsesWith(
          UndefValue::get(bb->getTerminator()->getType()));
    }
...
bool RADeadCodeElimination::verify_equal(Range r1, Range r2) {
  return r1.getUpper().eq(r2.getUpper()) && r1.getLower().eq(r1.getUpper()) &&
...
static RegisterPass<RADeadCodeElimination>
    X("dead-code-elimination", "A client that uses dead code elimination");
